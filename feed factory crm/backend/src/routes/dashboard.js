  ...
  try {
    const result = await query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM invoices WHERE due_date < NOW() AND status != 'paid' AND balance_due > 0) as overdue_invoices,
        (SELECT COALESCE(SUM(balance_due),0) FROM invoices WHERE due_date < NOW() AND status != 'paid' AND balance_due > 0) as overdue_amount,
        (SELECT COALESCE(SUM(balance),0) FROM supplier_payables WHERE status IN ('pending','partial','overdue')) as total_payables,
        (SELECT COALESCE(SUM(balance),0) FROM supplier_payables WHERE status IN ('pending','partial') AND due_date < CURRENT_DATE AND balance > 0) as overdue_payables,
